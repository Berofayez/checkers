...
## Done when
- [ ] After a jump, if the same piece has another legal jump, its turn does
      not end and only that piece's further jumps are highlighted.
- [ ] The player cannot switch to a different piece mid-chain.
- [ ] The chain continues until the jumping piece has no further legal jump,
      at which point the turn ends and play passes to the other player.
- [ ] Piece counts update correctly across a multi-jump.
